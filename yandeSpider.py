...
global driver, se, time_min, time_max, ranges, download_path, is_over, next_page_exist, google_data_path, download_fluency_control, headless, load_img
se = Session()
se.keep_alive = False
#全局变量初始化
ranges = 2  #存放页面下翻次数
...
def get_large_img_url(page_url = '', tag_str = ''):
    ...
    li_list = driver.find_elements_by_xpath(".//li[@style='width: 160px;']")
    # print(len(li_list))
    if len(li_list) == 0:
...
def download_large(pic_list = [], tag=""):  #url_dic = {'img_url_due': Queue(maxsize=0), 'id_que': Queue(maxsize=0), 'tag_name': Queue(maxsize=0)}
    ...
    while not (url_dic['img_url_due'].empty()):
        ...
        if "{0}.{1}".format(pic_name, format_pic) in pic_list:
            ...
            # pic_list.append("{0}.{1}".format(pic_name, format_pic))
            number = 0
            while True:
                try:
                    response = se.get(tar_url, stream=True, verify=False, timeout=20)   #必须要有stream控制，才能显示下载流进度
                    print("请求成功，response status: ", response.status_code)
                    break
                except:
                    number+=1
                    if number==50:
            ...
            with open("{0}.{1}".format(pic_name, format_pic), 'ab') as konanchan_pic:
                try:
                    ...
                    for data in response.iter_content(chunk_size=chunk_size):  # 一块一块下载
                        ...
                        # print("当前进度",now)
                        sys.stdout.write(
                            "\r[%s%s] %d%%" % ('⬛' * now, ' ' * (50 - now), 100 * data_count / content_size))
                        sys.stdout.flush()
                        # print("\r下载进度：： %d%%(%.2f/%.2f)" % (now, data_count / 1024, content_size / 1024), end="")  # \r是防止多次打印
                    if now == 50:
                        with open("{0}\\{1}\\{2}_picture_list.txt".format(download_path, art_file, art_file),
                                  'a') as pic_list_write:
                            pic_list_write.write("{0}.{1}\n".format(pic_name, format_pic))
                        print("\n下载成功..::{0} 下载成功...".format(tar_url))
...
def ini_config_read():
    ...
    #     dic = json.loads(conf)
    try:
        with open(os.getcwd() + "\\yande" + "\\config.json", 'r', encoding='utf8') as conf:
            conf = conf.read().replace('\\', '\\\\')
            print("conf:", conf)
            dic = json.loads(conf)
    except:
    ...
    try:
        with open(os.getcwd()+ "\\yande" + "\\tag_list.txt", 'r') as ID_f:
            for line in ID_f:
...
def Yan_main():
    global download_fluency_control, google_data_path, download_path, next_page_exist, auto_break, auto_break_picNum_exist, break_num_rec, driver, headless, load_img
    ...
    for tag in tag_list:
        break_num_rec = 0
        art_file = re.sub('[/\:,*?"<>|]', '', tag)  # 规范文件名
        if "ordervote" in art_file:
            art_file = "我的收藏"
        set_path(download_path, tag)
        pic_list = []
        if os.path.exists("{0}\\{1}\\{2}_picture_list.txt".format(download_path, art_file, art_file)):  # 读取已下载的文件列表
        ...
        while next_page_exist == True:
            if (break_num_rec >= auto_break_picNum_exist and auto_break):
                ...
                break
            if next_page_exist == True:
                i = i + 1
                print("正在解析 {0} 第 {1}页...".format(tag, i))  # https://yande.re/post?page=2&tags=mochizuki_shiina
                url_page = "https://yande.re/post?page=" + str(i) + "&tags=" + tag
                get_large_img_url(url_page, tag)
                download_large(pic_list, tag)
        # with open("{0}\\{1}\\{2}_picture_list_1.txt".format(download_path, tag, tag), 'w') as pic_list_write:  #再次记录
        #     for pic in pic_list:
